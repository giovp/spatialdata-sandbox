...
class ClonalTileDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clonal_information = self.tile_dataset.sdata.table[idx].obs["clone"].values[0]
...
def spatial_image_to_tensor(image: SpatialImage):
    # return torch.from_numpy(image.data.compute().astype(np.float32))
    return image
...
if __name__ == "__main__":
    ##
    f = "/home/l989o/pycharm_project_143/logs/lightning_logs/version_9/checkpoints/epoch=29-step=210.ckpt"
    model = DenseNetModel.load_from_checkpoint(f)
